...
export function isEditorImageFile(file: File, fallbackType = '') {
...
function pastedImageFile(event: PasteEventLike) {
  ...
  for (const item of items) {
    ...
    if (file && isEditorImageFile(file, item.type)) return file
  }
...
export function hasPastedImageFile(event: PasteEventLike) {
  return Boolean(pastedImageFile(event))
}
