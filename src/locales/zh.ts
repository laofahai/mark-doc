export default {
  app: {
    name: 'MarkDoc',
    editor: '编辑器',
    settings: '设置',
  },
  common: {
    cancel: '取消',
    save: '保存',
    export: '导出',
    newFile: '新建',
    open: '打开文件',
    openFolder: '打开文件夹',
    closeFolder: '关闭文件夹',
    parentFolder: '上级目录',
    drillInto: '进入此目录',
    untitled: '未命名.md',
    chars: '字',
    language: '语言',
    theme: '主题',
    done: '完成',
    untitledMdoc: '未命名.mdoc',
  },
  fileFilters: {
    markdocPackage: 'MarkDoc 文档包',
    markdown: 'Markdown 文档',
    text: '文本文件',
    word: 'Word 文档',
    wordTemplate: 'Word 样式模板',
  },
  toolbar: {
    exportPdf: '保存为 PDF（系统打印）',
    documentCommands: '文档命令',
    file: '文件',
    newFile: '新建',
    openFile: '打开文件...',
    openFolder: '打开文件夹...',
    recent: '最近',
    clearRecent: '清空最近文件',
    removeRecent: '从最近文件移除 {{name}}',
    save: '保存',
    saveShortcut: '保存 (⌘S)',
    export: '导出',
    exportMd: '导出 .md',
    exportDocx: '导出 .docx',
    editorView: '视图',
    viewFit: '适合窗口',
    viewActual: '实际大小',
    viewWide: '宽屏画布',
    pageSetup: '页面设置',
    print: '打印',
    pageSizeA4: 'A4',
    pageSizeLetter: 'Letter',
    orientationPortrait: '纵向',
    orientationLandscape: '横向',
  },
  sidebar: {
    outline: '大纲',
    files: '目录',
    openFolder: '打开文件夹',
    noOutline: '当前文档没有标题',
    collapseAll: '全部折叠',
    expandAll: '全部展开',
    collapseHeading: '折叠标题',
    expandHeading: '展开标题',
  },
  editor: {
    resetZoom: '恢复至 100% 缩放',
    largeDocument: '大文件',
    documentMode: '文档视图',
    sourceMode: '源码',
    formattedMode: '排版',
    loading: '正在打开文档…',
    largeFormattedConfirm: '此文档较大，完整排版可能需要较长时间并暂时影响响应。继续切换？',
    placeholder: '开始编写...',
    startEditing: '打开文件或新建文件开始编辑',
    externalChange: '「{{fileName}}」已被外部程序修改',
    reload: '重新加载',
    ignore: '忽略',
    formattingToolbar: '编辑工具栏',
    color: '颜色',
    textColor: '文字颜色',
    backgroundColor: '背景颜色',
    clearTextColor: '清除文字颜色',
    clearBackgroundColor: '清除背景颜色',
    clearFormatting: '清除格式',
    linkUrl: '链接地址',
    emoji: '表情',
    toolbar: {
      heading: '标题',
      bold: '加粗',
      italic: '斜体',
      strike: '删除线',
      blockquote: '引用',
      bulletList: '无序列表',
      orderedList: '有序列表',
      taskList: '任务列表',
      inlineCode: '行内代码',
      codeBlock: '代码块',
      link: '链接',
      table: '表格',
      horizontalRule: '分割线',
      uploadImage: '上传图片',
    },
    emojiGroups: {
      people: '表情',
      gestures: '手势',
      marks: '标记',
      work: '工作',
      objects: '物件',
    },
  },
  settings: {
    tabAbout: '关于',
    aboutDescription: 'Markdown 文档编辑，正文与图片一起放进一个 .mdoc 文件。',
    about: '关于 MarkDoc',
    presentBy: '由 Linch Tech 开发',
    website: '官网',
    title: '设置',
    tabGeneral: '通用',
    tabTemplate: '文档模板',
    updateTitle: '应用更新',
    currentVersion: '当前版本 {{version}}',
    checkForUpdates: '检查更新',
    checkingUpdates: '检查中...',
    upToDate: '已是最新版本',
    updateAvailable: '发现新版本 {{version}}',
    releaseDate: '发布时间 {{date}}',
    downloadAndRestart: '下载并重启',
    downloadingUpdate: '下载中 {{percent}}%',
    restartingUpdate: '正在重启...',
    updatesDesktopOnly: '更新检查仅在桌面应用中可用。',
    updateBlockedByUnsavedChanges: '请先保存已打开的文档，再安装更新。',
    updateFailed: '更新失败：{{message}}',
    templateDesc: '导出 Word 文档时使用的样式模板（reference.docx）。模板控制字体、页边距、页眉页脚等样式。',
    defaultTemplate: '日常文档',
    defaultTemplateDesc: '10.5pt 无衬线正文，紧凑标题，1.6 倍行距',
    customTemplate: '自定义模板',
    customTemplateDesc: '上传你自己的 reference.docx。在 Word 中设置好样式（字体、行距、页眉页脚）后保存即可作为模板。',
    selectTemplateFile: '选择模板文件',
  },
  export: {
    dailyTemplate: '日常文档',
    dailyTemplateDesc: '10.5pt 无衬线正文，紧凑标题，1.6 倍行距',
    formalTemplate: '正式文档',
    formalTemplateDesc: '宋体正文、黑体标题，适合正式文件',
    title: '导出 Word 文档',
    selectTemplate: '选择文档样式模板',
    keepOriginalStyle: '保留原文件样式',
    keepOriginalStyleDesc: '使用打开的 Word 文件自身的样式',
    defaultTemplate: '日常文档',
    defaultTemplateDesc: '与编辑器默认风格一致：紧凑标题、10.5pt 正文',
    customTemplate: '自定义模板',
    selectOtherTemplate: '选择其他模板文件...',
    saveTo: '保存位置',
    selectSavePath: '选择保存路径...',
  },
  document: {
    saveAsMarkDoc: '保存为 MarkDoc 文档',
    suggestPackage: '建议保存为 MarkDoc 文档以便打包资源',
  },
  workspace: {
    recoveryAvailable: '检测到可恢复的文档草稿',
  },
  assets: {
    extractEmbeddedImages: '提取内嵌图片',
    remoteBlocked: '远程资源已被阻止',
  },
  presentation: {
    chineseFormal: '中文正式文档',
    referenceDocx: 'Word 样式模板',
  },
  import: {
    docxAsWorkspace: 'Word 文档已导入为可编辑文档',
  },
  package: {
    invalidManifest: 'MarkDoc 文档清单无效',
    corruptedRecovery: '文档包已损坏，已进入安全恢复模式',
    quarantinedResources: '部分文档包资源已被隔离',
    missingManifestResources: '文档包缺少清单声明的资源',
  },
  security: {
    enableRemoteForDocument: '允许此文档加载远程资源',
    enableRemoteImages: '允许远程图片',
    enableRemoteStyles: '允许远程样式',
    enableRemoteFonts: '允许远程字体',
    enableRemoteScripts: '允许远程脚本',
    enableOtherRemoteResources: '允许其他远程资源',
    allowRemoteDomain: '允许 {{domain}}',
    allowRemoteUrl: '允许此 URL',
    quarantineResource: '此资源已被隔离',
  },
  recovery: {
    retrySave: '重试保存',
    saveAs: '另存为',
    restoreDraft: '恢复草稿',
    discardDraft: '丢弃草稿',
  },
  errors: {
    open: {
      failed: '打开文档失败：{{path}}',
    },
    workspace: {
      noRoot: '文档工作区没有文件系统根目录',
    },
    package: {
      invalidManifest: 'MarkDoc 文档清单无效：{{path}}',
      openFailed: '打开 MarkDoc 文档包失败：{{path}}',
      unsafePath: '文档包包含不安全路径：{{path}}',
      limitExceeded: 'MarkDoc 文档包超出安全资源限制：{{path}}',
      assetCopyFailed: '保存失败：无法打包资源 {{path}}，当前内容已保留在恢复草稿中',
    },
    save: {
      failed: '保存失败，但当前内容已保留在恢复草稿中',
    },
    import: {
      docxFailed: '导入 Word 文档失败',
    },
    export: {
      docxFailed: '导出 Word 文档失败',
    },
  },
  close: {
    title: '未保存的更改',
    message: '「{{fileName}}」有未保存的更改，是否保存？',
    discard: '不保存',
  },
  pandoc: {
    title: '需要安装 Pandoc',
    desc: 'Pandoc 是文档转换引擎，用于 Word 文档的导入导出。请先安装后使用。',
    installing: '安装中...',
    autoInstall: '自动安装',
    recheck: '重新检测',
    noHomebrew: '未检测到 Homebrew，请先安装 Homebrew 后重试，或手动安装 Pandoc',
    linuxManual: 'Linux 需要手动安装，请参考下方命令',
    macosGuide: '请打开终端运行：brew install pandoc\n\n如果没有 Homebrew，请先访问 https://brew.sh 安装',
    windowsGuide: '请打开 PowerShell 运行：winget install -e --id JohnMacFarlane.Pandoc',
    linuxGuide: '请打开终端运行：sudo apt install pandoc（Ubuntu/Debian）\n或：sudo dnf install pandoc（Fedora）',
    unknownGuide: '请访问 https://pandoc.org/installing.html 下载安装',
  },
}
